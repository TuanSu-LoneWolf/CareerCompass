...
// Import các hàm và biến cần thiết từ auth.js
// import { auth, database, signInWithEmailAndPassword, GoogleAuthProvider, signInWithPopup, ref, update } from '../auth.js';
// import { onAuthStateChanged } from "https://www.gstatic.com/firebasejs/11.6.0/firebase-auth.js";
import { auth, database, signInWithEmailAndPassword, GoogleAuthProvider, signInWithPopup, ref, update, onAuthStateChanged } from '../auth.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Lấy tham chiếu đến các phần tử điều hướng
    ...
    function handleLoginSuccess(user) {
        ...
        if (signupLink && loginLink && userLogoContainer && userLogo) {
            ...
            userLogo.src = user.photoURL || './default-user.png'; // Hiển thị ảnh người dùng hoặc ảnh mặc định
        ...
        googleLoginBtn.addEventListener('click', () => {
            ...
            signInWithPopup(auth, provider)
                .then((result) => {
                    ...
                    // Kiểm tra xem người dùng có mới không và ghi thông tin cơ bản vào DB nếu cần
                    const userRef = ref(database, 'users/' + user.uid);
                    update(userRef, { // Sử dụng update thay vì set để không ghi đè các thông tin khác
                        email: user.email,
                        displayName: user.displayName || 'Người dùng Google', // Tên hiển thị
                        photoURL: user.photoURL || '', // Ảnh đại diện
                        provider: 'google.com', // Đánh dấu là đăng nhập từ Google
                        ...
                        console.error("Lỗi khi ghi/cập nhật dữ liệu người dùng Google:", dbError);
                        // Vẫn gọi handleLoginSuccess để cập nhật giao diện
                    }).finally(() => {
                        handleLoginSuccess(user); // Gọi hàm xử lý chung sau khi (cố gắng) cập nhật DB
                    });
                })
                .catch((error) => {
                    // Xử lý lỗi khi đăng nhập Google
    ...
    onAuthStateChanged(auth, (user) => {
        console.log("Trạng thái đăng nhập thay đổi:", user); 
        if (user) {
